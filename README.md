...
### 3. 编码器架构对比（GRU vs LSTM vs Transformer）

#### 3.1 整体指标对比

![架构对比 — 整体](results/arch_comparison_overall.png)

分组柱状图展示三种编码器在 RMSE、MAE、CRPS、Winkler Score 和 Coverage@90% 上的表现。GRU 在点预测和概率预测指标上全面领先，Transformer 的 Coverage 虽然最高但远超名义值 0.90，说明预测区间过宽。

#### 3.2 逐 Horizon RMSE 与 CRPS

![架构对比 — 逐 Horizon](results/arch_comparison_horizon.png)

逐预测时刻的 RMSE 和 CRPS 曲线。三种架构的差距随 horizon 增大而逐渐扩大，GRU 在所有时刻上保持最优。LSTM 与 GRU 在短 horizon（1-6h）差距极小，但在长 horizon（>18h）差距增大。

#### 3.3 不确定性分解

![架构对比 — 不确定性](results/arch_comparison_uncertainty.png)

Transformer 的 aleatoric 和 epistemic 不确定性在所有 horizon 上都显著高于 GRU 和 LSTM，且随 horizon 增大差距进一步拉开。这说明 Transformer 在该数据规模下对自身预测缺乏信心，不确定性估计偏保守。GRU 和 LSTM 的 aleatoric 不确定性非常接近，但 GRU 的 epistemic 不确定性略低。

#### 3.4 Skill Score 对比

![架构对比 — Skill Score](results/arch_comparison_skill.png)

三种架构在所有 horizon 上都优于 persistence 基线（Skill > 0），且在长 horizon 优势更大。GRU 全程领先，在 h=24 处仍保持约 0.7 的 Skill Score。

#### 3.5 雷达图

![架构对比 — 雷达图](results/arch_comparison_radar.png)

归一化雷达图（外侧更优）。GRU 的多边形面积最大，在所有维度上表现最均衡。Transformer 在多数维度上缩至内圈，仅 Coverage 维度略优（但这实际是预测区间过宽的副效应）。
...
#### 6.2 CRPS 与 Winkler Score（以青岛为例）
...
### 3. Encoder Architecture Comparison (GRU vs LSTM vs Transformer)

#### 3.1 Overall Metrics

![Architecture Comparison — Overall](results/arch_comparison_overall.png)

Grouped bar charts comparing three encoder architectures across RMSE, MAE, CRPS, Winkler Score, and Coverage@90%. GRU leads in all point and probabilistic forecast metrics. Transformer's Coverage exceeds the nominal 0.90 level, indicating overly wide prediction intervals.

#### 3.2 Per-Horizon RMSE & CRPS

![Architecture Comparison — Horizon](results/arch_comparison_horizon.png)

Per-horizon RMSE and CRPS curves. The gap between architectures widens with forecast horizon. LSTM is close to GRU at short horizons (1-6h) but diverges at longer horizons (>18h).

#### 3.3 Uncertainty Decomposition

![Architecture Comparison — Uncertainty](results/arch_comparison_uncertainty.png)

Transformer exhibits significantly higher aleatoric and epistemic uncertainty at all horizons, suggesting it lacks confidence in its predictions at this data scale. GRU and LSTM share similar aleatoric uncertainty, but GRU has slightly lower epistemic uncertainty.

#### 3.4 Skill Score Comparison

![Architecture Comparison — Skill](results/arch_comparison_skill.png)

All three architectures outperform the persistence baseline across all horizons (Skill > 0), with greater advantage at longer horizons. GRU leads throughout, maintaining ~0.7 Skill Score at h=24.

#### 3.5 Radar Chart

![Architecture Comparison — Radar](results/arch_comparison_radar.png)

Normalized radar chart (outer = better). GRU shows the largest polygon area with the most balanced performance. Transformer shrinks to the inner region on most dimensions.

---

### 4. Ablation Study

#### 4.1 Ablation Metrics
...
#### 5.1 Uncertainty Decomposition Across Sites
...
#### 5.4 Sample Uncertainty Decomposition (Qingdao)
...
#### 6.1 Horizon Diagnostics (Qingdao)
...
#### 6.2 CRPS & Winkler Score (Qingdao)
...
#### 6.3 Skill Score vs Horizon (Qingdao)
...
#### 6.4 Error Distribution (Qingdao)
